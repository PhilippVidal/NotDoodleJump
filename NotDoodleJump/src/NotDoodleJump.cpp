...
Platform* NotDoodleJump::SpawnPlatform(EPlatformTypes platformType)
{
	...
	switch (platformType)
	{
		case EPlatformTypes::MOVING:
		{
			...
			platformConstrData.Scale = Vec2f(3.75f, 3.75f);
...
Enemy* NotDoodleJump::SpawnEnemy(Platform* platform)
{
	...
	if (rand() % 2)
	{
		...
		entityConstrData.Scale = Vec2f(2.f, 2.f);
	}
	else
	{
		...
		entityConstrData.Scale = Vec2f(2.25f, 2.25f);
	}
...
AbilityPickup* NotDoodleJump::SpawnAbility(Platform* platform)
{
	...
	pickupConstrData.Scale = Vec2f(1.7f, 1.7f);
...
Blackhole* NotDoodleJump::SpawnBlackhole(float minHeight, float maxHeight)
{
	...
	constrData.Position = Vec2f(Math::GetRandomFloat(0.f, GetGame()->GetWorldSize().x), Math::GetRandomFloat(minHeight, maxHeight));
	constrData.Scale = Vec2f(5.f, 5.f);
...
void NotDoodleJump::PostInit()
{
	...
	playerConstrData.ColliderType = EColliderTypes::CIRCLE;
	playerConstrData.CollisionLayer = ECollisionLayers::PLAYER;
	playerConstrData.Position = worldCenter;
	playerConstrData.Scale = Vec2f(1.525f, 1.525f);
	...
	//Crosshair
	{
		...
		crosshairConstrData.ScreenScale = Vec2f(2.63f, 2.63f);
	...
	//Score Widgets
	{
		...
		scoreConstrData.ScreenScale = Vec2i(5.f, 5.f);
		...
		screenPosition.y = topRightCorner.y + (platformsPassedWidgetSize.y / 2) + distanceWidgetSize.y + verticalOffset;
		...
		screenPosition.y = topRightCorner.y + (m_ScoreWidgetCointsCollected->GetScreenSize().y / 2) + distanceWidgetSize.y + platformsPassedWidgetSize.y + verticalOffset * 2;
